...
class ImageManipulation {
    ...
    public Mat canny(Mat srcImage) {
        ...
        Imgproc.Canny(mGray, mGray, 5, 5, 3, true);

        return mGray;
    }

    public Mat segmentImageScreen (Mat srcImage) {
        Mat mRgba = srcImage.clone();
        Mat mGray = new Mat();
        ...
        matList  = new ArrayList<Mat>();
//        System.out.println(contour2.size());

        for (int i = 0; i < contour2.size(); i++) {
            rectan3 = Imgproc.boundingRect(contour2.get(i));

            if (rectan3.area() > 0.5 * imgsize || rectan3.area() < 5000 || rectan3.width / rectan3.height < 3) {
                Mat roi = new Mat(morbyte, rectan3);
                roi.setTo(zeos);
            } else {
                Imgproc.rectangle(mRgba, rectan3.br(), rectan3.tl(), new Scalar(0, 0, 255));
